class PaymentsController < ApplicationController
  ...
  def create
    ...
    if @payment.can_be_saved? 
      @payment.save
      flash[:notice] = "Yes, payment of <b>#{@payment.amount}</b> is successful! "+ 
                        "Transaction code = <b>#{@payment.transaction_code}</b>"
      
    else
      flash[:error] = "Ah no! the amount entered is exceeds the remaining payment. It should't exceed " + 
